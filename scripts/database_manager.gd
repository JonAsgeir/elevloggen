...
func get_student_history(student_id: int, subject_id: int, filter: String = "all") -> Array:
	...
	match filter:
		"goals":
			filter_sql = "WHERE event_type = 'goal'"
		"interactions":
			filter_sql = "WHERE event_type = 'interaction'"
		"mastery":
			filter_sql = "WHERE event_type = 'mastery'"
		"competence":
			filter_sql = "WHERE event_type = 'competence'"
		_:
			filter_sql = ""
	...
	FROM (
		...
		AND (
			...
			OR student_competence_assessments.comment != ''
		)
	) AS history_events
	%s
